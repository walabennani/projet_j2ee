...
public class EquipementController {
    ...
    public ResponseEntity<Equipement> updateEquipement(@PathVariable("id") Long id, @RequestBody Equipement equipement) {
        Optional<Equipement> existing = equipementService.getEquipementById(id);
        if (existing.isPresent()) {
            equipement.setId(id); // pour s'assurer qu'on met à jour le bon
            Equipement updated = equipementService.saveEquipement(equipement);
