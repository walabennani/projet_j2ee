...
public class InterventionController {
    ...
    public ResponseEntity<?> createIntervention(@Valid @RequestBody Intervention intervention) {
        try {
            ...
            logger.error("Erreur lors de la création de l'intervention", e);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateIntervention(@PathVariable("id") Long id, @Valid @RequestBody Intervention interventionDetails) {
        try {
            Optional<Intervention> optionalIntervention = interventionService.getInterventionById(id);
            if (optionalIntervention.isEmpty()) {
                return ResponseEntity.notFound().build();
            }

            Intervention existing = optionalIntervention.get();
            existing.setDate(interventionDetails.getDate());
            existing.setCost(interventionDetails.getCost());
            existing.setStatus(interventionDetails.getStatus());
            existing.setEquipment(interventionDetails.getEquipment());
            existing.setTechnician(interventionDetails.getTechnician());
